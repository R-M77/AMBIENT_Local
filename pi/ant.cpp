...
{
    std::string command = "python3 video_split.py start " + tagID;
    system(command.c_str());
}

void stop_recording()
{
    std::string command = "python3 video_split.py stop";
    system(command.c_str());
}
...
public:
    static void run(ReaderModule &reader)
    {
        ...
        while (elapsedSeconds.count() < 400.0)
        {
            ...
            // create map  to store tag count
            map<string, int> currentTagsCount;
            ...
            for (auto &entry : recordingStatus)
            {
                const string &tagID = entry.first;
                bool &isRecording = entry.second;

                int tagCount = currentTagsCount[tagID];

                if (tagCount == 1 && !isRecording)
                {
                    // If the tag is read for the first time and not recording, start recording
                    isRecording = true;
                    start_recording(tagID);
                }
                else if (tagCount >= 2 && isRecording)
                {
                    // If the tag is read for the second time or more and is recording, stop recording
                    isRecording = false;
                    stop_recording();
                }
            }

...
// std::map<std::string, bool> recordingStatus;
std::map<std::string, std::chrono::steady_clock::time_point> lastScanTime;
    ...
    static void run(ReaderModule &reader)
    {
        ...
        while (elapsedSeconds.count() < 4000.0)
        {
            ...
            for (size_t itemIndex = 0; itemIndex < count; itemIndex++)
            {
                ...
                auto lastScanIt = lastScanTime.find(iddString);
                ...
                if (it == recordingStatus.end())
                {
                    ...
                    lastScanTime[iddString] = now;
                    start_recording(iddString);
                }
                else
                {
                    // Check if the tag was scanned within the last 1 second
                    std::chrono::duration<double> timeSinceLastScan = now - lastScanIt->second;
                    if (timeSinceLastScan.count() < 1.0)
                    {
                    ...
                    // Update the last scan time for the tag
                    lastScanTime[iddString] = now;
                }
            ...
            //     string iddString = tagItem.iddToHexString();
            //     std::cout << "IDD: " << iddString << std::endl;
            //     // check if the same tag id is scanned for the first time
            //     if (recordingStatus.find(iddString) == recordingStatus.end())
            //     {

...
# Purpose: Record video from 2 cameras simultaneously, split the video into 2 parts, and send the 2 parts to the remote server
# Usage: python3 2tags_vid_wip.py start <tagID> to start recording
#        python3 2tags_vid_wip.py split <tagID> <tag2_start> <tag1_duration> <tag2_duration> to split the video
#        python3 2tags_vid_wip.py send to send the 2 parts to the remote server
#        python3 2tags_vid_wip.py stop to stop recording
# 
# Note: The video file name is in the format of <tagID>_<YYYYMMDD-HHMMSS>_cam1.h264
#       The 2 parts of the video file are in the format of <tagID>_<YYYYMMDD-HHMMSS>_cam1_tag1.h264 and <tagID>_<YYYYMMDD-HHMMSS>_cam1_tag2.h264
#       The 2 parts of the video file are in the format of <tagID>_<YYYYMMDD-HHMMSS>_cam2_tag1.h264 and <tagID>_<YYYYMMDD-HHMMSS>_cam2_tag2.h264
...
def split_video(source_filename, start_time, duration, destination_filename): #DONE
    # command = ['ffmpeg', '-i', source(start_time), '-t', '00:00:' + str(duration), destination_filename]
    
    command = ['ffmpeg', '-i', source_filename, '-ss', '00:00:' 
               + str(start_time), '-t', '00:00:' + str(duration), 
               '-codec:v', 'copy', '-codec:a', 'copy', '-f', 
               'h264', destination_filename]
...
def main(action, tagID=None, tag2_start=None, tag1_duration=None, tag2_duration=None):
    if action == "split" and tagID is not None and tag2_start is not None and tag1_duration is not None and tag2_duration is not None:
        # video_file = f"2ScanCombined.h264"
        video_file1 = video_file
        video_file2 = f"{tagID}_{time.strftime('%Y%m%d-%H%M%S')}"
        source_filename1 = video_file1 + "_cam1.h264"
        # source_filename2 = video_file + "_cam2.h264"
        source_filename2 = video_file2 + "_cam2.h264"
        global destination_filename1_1, destination_filename1_2, destination_filename2_1, destination_filename2_2
        destination_filename1_1 = video_file1 + "_cam1_tag1.h264"
        destination_filename1_2 = video_file2 + "_cam1_tag2.h264"
        destination_filename2_1 = video_file1 + "_cam2_tag1.h264"
        destination_filename2_2 = video_file2 + "_cam2_tag2.h264"
